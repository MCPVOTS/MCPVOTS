...
class EthermaxDashboard {
    ...
    time: number;
    dataConnector: any;
    pulseEffect: { scale: number; direction: number };
    centralToken: any;
    ...
    setupDataConnector() {
        ...
        this.dataConnector = new (window.EthermaxDataConnector || class {
            mockDataInterval: NodeJS.Timeout | null = null;
            tokenData: TokenData | null = null;
            swarmData: SwarmData | null = null;
            networkData: NetworkData | null = null;
            onTokenDataUpdate: ((data: any) => void) | null = null;
            onSwarmDataUpdate: ((data: any) => void) | null = null;
            onNetworkDataUpdate: ((data: any) => void) | null = null;
            onTransactionUpdate: ((data: any) => void) | null = null;
            ...
            updateUI() {
                // This will be called by the dashboard
            }

            subscribeToStream(streamName: string, callback: (data: any) => void) {
        ...
        this.dataConnector.subscribeToStream('tokenData', (data: any) => {
            this.tokenData = { ...this.tokenData, ...data };
            this.updateUI();
        });

        this.dataConnector.subscribeToStream('swarmData', (data: any) => {
            this.swarmData = { ...this.swarmData, ...data };
            this.updateUI();
        });

        this.dataConnector.subscribeToStream('networkData', (data: any) => {
    ...
    updateUI() {
        ...
        const swarmsElement = document.getElementById('swarms-value');
        const botsElement = document.getElementById('bots-value');
        const networkLoadElement = document.getElementById('network-load');
        const activeBotsElement = document.getElementById('active-bots');
        const txPerMinElement = document.getElementById('tx-per-min');
        const confidenceScoreElement = document.getElementById('confidence-score');

        if (priceElement) priceElement.textContent = `$${this.tokenData.price.toFixed(6)}`;
        if (volumeElement) volumeElement.textContent = `$${(this.tokenData.volume / 1000).toFixed(0)}K`;
        if (swarmsElement) swarmsElement.textContent = this.swarmData.activeSwarms;
        if (botsElement) botsElement.textContent = this.swarmData.botAddresses;
        if (networkLoadElement) networkLoadElement.textContent = `${this.swarmData.confidenceScore}%`;
        if (activeBotsElement) activeBotsElement.textContent = this.networkData.activeBots;
        if (txPerMinElement) txPerMinElement.textContent = this.networkData.txPerMin;
        if (confidenceScoreElement) confidenceScoreElement.textContent = `${this.swarmData.confidenceScore}%`;
    }
    ...
    animate() {
        ...
        this.swarms.forEach(swarm => {
            ...
            swarm.children.forEach((child: any, index: number) => {
    ...
    dispose() {
        ...
        if (this.dataConnector && this.dataConnector.mockDataInterval) {
            clearInterval(this.dataConnector.mockDataInterval);
        }
    }
}
...
export default function ThreeJSAnimation() {
    ...
    useEffect(() => {
        if (containerRef.current && !dashboardRef.current) {
            dashboardRef.current = new EthermaxDashboard(containerRef.current);
        }
